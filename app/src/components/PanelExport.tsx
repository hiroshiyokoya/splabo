 ...
 * - `PanelExportLogo`   … 画像にだけ入る右上の splabo ロゴと右下のクレジット。
 *   画面では非表示（`.is-exporting` のときだけ表示）なので、通常のレイアウトは変わらない。
 *   クレジット本文（版・日付）はキャプチャ直前に `savePanelAsPng` が埋める。
 */
import { useState, type RefObject } from 'react'
import { EXPORT_HIDE_CLASS, savePanelAsPng } from '../utils/panelExport'
...
export function PanelExportButton({ targetRef, screen, panel }: ButtonProps) {
  ...
  async function handleClick() {
    ...
    setBusy(true)
    try {
      await savePanelAsPng(node, screen, panel)
    } catch (e) {
...
/**
 * 画像に焼き込むブランディング。右上に splabo ロゴ、右下にクレジット枠。
 * クレジット文言（`Generated by SpLabo vX.Y.Z · YYYY-MM-DD`）は
 * キャプチャ直前に `savePanelAsPng` が `.panel-export-credit` へ書き込む。
 */
