...
export const ABILITY_LABELS: Record<string, string> = {
  ...
  run_speed_up:      'ヒト移動速度',
  swim_speed_up:     'イカダッシュ速度',
  special_charge_up: 'スペシャル増加量',
  special_saver:     'スペシャル減少量',
  special_power_up:  'スペシャル性能',
  quick_respawn:     '復活時間短縮',
  quick_super_jump:  'スーパージャンプ時間短縮',
  sub_power_up:      'サブ性能',
  ink_resistance_up: '安全靴',
  ...
  ability_doubler:   'サブ性能アップ系の効果倍化', // (旧名: アビリティダブラー)
  stealth_jump:      'ステルスジャンプ',
  object_shredder:   'オブジェクト破壊力アップ',
  drop_roller:       'リベンジ着地', // (旧名: ジャンプ短縮)
  empty:             '未付与',
}
 ...
 * 未キャッシュは Map に含まれない。
 */
export async function loadAbilityImages(): Promise<Map<string, string>> {
  const entries = await Promise.all(
    ABILITY_KEYS.map(key =>
      invoke<string | null>('read_image', { kind: 'ability', name: key })
...
/**
 * 画像 URL からアビリティキーを推定する。
 * URL に含まれるハッシュをルックアップ。未知は null。
 */
export function abilityKeyFromUrl(url: string | undefined | null): string | null {
  if (!url) return null
  for (const [hash, key] of URL_HASHES) {
    if (url.includes(hash)) return key
  }
  return null
}

// Rust 側 abilities.rs::ABILITY_HASHES と同じ並び。
// 空スロット画像は 'empty' に正規化。
const URL_HASHES: [string, string][] = [
  ...
  ['dc937b59892604f5a86ac96936cd7ff09e25f18ae6b758e8014a24c7fa039e91', 'empty'],
]

/** RGBA Color から CSS hex 文字列に変換（`#rrggbb`）。 */
export function colorToHex(c: { r: number; g: number; b: number } | undefined): string | undefined {
