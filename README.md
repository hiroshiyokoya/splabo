...
- **モバイル同期（コンパニオン）** — 設定で有効にすると、同じ Wi-Fi のスマホアプリ [splabo-viewer](https://github.com/hiroshiyokoya/splabo-viewer) へ QR ペアリングでギア・直近バトル・画像を配信します。オプトイン・アプリ起動中のみ動作し、任天堂 API へのアクセスはデスクトップ側のみで完結します（モバイルは任天堂 API に一切アクセスしません）
