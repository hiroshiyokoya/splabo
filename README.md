...
| ② 見せ方を決める | 行・列・セルの中身 | 質問文・列名・**集計結果の先頭 20 行**（ステージ名・ブキ名・勝率などの集計値） |
| アプリ | 表の組み立て | （送信なし。ピボットとセルの整形は Rust の `ai_present` が行う） |

②が返すのは「どの列を行に置くか」の指定だけで、**数値には触れません**。表の形はアプリが作るので、AI が形を守らなくても崩れません。

表示は「AI の説明」「実行した SQL」「表」「元の集計結果」です。プロバイダごとに価格情報付きのモデルプリセットを用意しています（`app/src/utils/aiModels.ts`）。API キーは `localStorage` に保存されます。

結果をグラフにする機能は次の段で入れる予定です（現状は表のみ）。
...
- **AI 分析 API（OpenAI / Gemini / Anthropic / Grok）**: AI 分析機能を使用する場合、選択したプロバイダへ 2 回リクエストします。
  ...
  - **② 表の形を決めさせるとき**: 質問文・列名と、**集計結果の先頭 20 行**。ステージ名・ブキ名・勝率・件数といった**集計値が含まれます**（個々のバトルの記録や他プレイヤーの名前は含みません）。
