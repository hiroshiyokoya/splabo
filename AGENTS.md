...
上の選択で下の項目の意味が変わる画面（グラフ設定モーダルなど）では、**使えない項目を出さない**。

- **意味を失った項目は消す。** 灰色にして並べても選べないだけで、場所を取って探しにくくなる。
  理由は**それを決めた項目のヒント**に書く（例:「点の見た目」の下に「サイズ・色は使えません」）。
- **例外は「その場で選べるようになる」もの。** 他の選択を外せば有効になる項目は、
  消すと関係が見えなくなるので、残して理由を添える（例: 折れ線で 3 軸目になるメトリクス）。
- 選択肢が 1 つしか無い select は、項目ごと消すか固定値の表示にする。開いても選べない。

## UI 文言（i18n）

新規のユーザー向け文言は `app/src/i18n/locales/ja.json` と `en.json` のキー経由にする（ハードコードしない）。表示言語は設定 → 表示 → 表示言語。
...
- **やったことだけを書く。経緯・原因・理屈は書かない。** 「これまでは〜だったため」「〜が積み上がって」のような背景説明を足さない。
  なぜそうしたかはイシューと PR に残っている。CHANGELOG は変わったことだけを短く。
