...
`tracking` ラベルのついたイシュー（現在: **#101**）は Claude が常に最新の状態に保つ。

**単一のトラッキングイシューに全マイルストーンを統合**して運用する。
マイルストーン（v1.0 / v1.1 / v1.2 / v1.3 / maintenance など）ごとにセクションを分け、
過去の完了マイルストーンも履歴として残す。

イシューの状態は GitHub が自動で付けるオープン／クローズのマークで判別するため、
本文中にアイコンやチェックボックスは付けない。クローズされたイシューも履歴として残す。
...
# → 出力された URL から Issue 番号を控える（例: 104）

# 2) 直後にトラッキング #101 へ追記する
gh issue view 101 --json body --jq .body > /tmp/tracking.md
# /tmp/tracking.md を編集して該当マイルストーンに「- #104 タイトル」を追記
